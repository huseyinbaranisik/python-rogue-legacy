...
GOLDEN_CHANCE      = 0.20
ENEMY_POWER_RATIO  = 0.96   # Canavarlar %25 güçlendirildi (0.77 * 1.25)
BOSS_POWER_RATIO   = 1.12   # Bosslar %25 güçlendirildi (0.90 * 1.25)

# ---------------------------------------------------------------------------
# YARDIMCI: Potion bilgisi (main.py'den buraya taşındı)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SAVAS SINIFI — Game'in içine mixin olarak inject edilir
# ---------------------------------------------------------------------------
class SavasMixin:
    ...
    def player_select_action(self, enemy):
        """Oyuncunun hamlesini seçer, doğrudan uygulamaz, dict döner."""
        oyuncu = self.oyuncu
        # Sersemleme kontrolü
    ...
    def turn_enemy(self, enemy):
        ...
        if isinstance(enemy, Worm):
            ...
            if target == self.oyuncu: self.oyuncu.kanama_uygula(yukler=1, sure=3)
    ...
    def _run_battle_loop(self, enemy):
        """Normal veya Boss savaşı için ortak döngü mantığı."""
        self.battle_log = []
    ...
    def _normalize_enemy_power(self, enemy, ratio=ENEMY_POWER_RATIO):
        """Düşmanı oyuncuya oransal güce normalize eder.
        Kişilik (iç stat oranları) korunur, sadece toplam güç ölçeklenir."""
    ...
    def battle_phase(self):
        ...
        enemy = rastgele_slime_getir(self.oyuncu.kat) if enemy_type == Slime else enemy_type(self.oyuncu.kat)

        # Güç normalizasyonu — eski lv*35/lv*8/lv*4 yerine
        self._normalize_enemy_power(enemy, ratio=ENEMY_POWER_RATIO)

        # Ödülleri kat'a göre ölçekle (ayrı, stat'tan bağımsız)
        floor_mult = 1.0 + (self.oyuncu.kat * 0.03)
        enemy.altin_odulu = max(5,  int(enemy.altin_odulu * floor_mult))
        enemy.tp_odulu    = max(10, int(enemy.tp_odulu    * floor_mult))
    ...
    def boss_battle(self, boss_class):
        ...
        # Bosslar %90 güçte + biraz ekstra can (daha uzun dövüş hissi)
    ...
    def handle_battle_end(self, enemy):
        ...
        os.system("cls" if os.name == "nt" else "clear")
        if self.oyuncu.hayatta_mi():
            floor_bonus = 1.0 + (self.oyuncu.kat * 0.05)
            gold = int(enemy.altin_odulu * floor_bonus)
            ...
            # Seviye atlama kontrolü - TP kaybını önlemek için while loop ve çıkarma işlemi
            while self.oyuncu.tecrube >= (self.oyuncu.seviye * 35):
                tecrube_gereksinimi = self.oyuncu.seviye * 35
